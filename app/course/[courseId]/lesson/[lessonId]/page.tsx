...
import { ArrowLeft, Check, CheckCircle, Circle, FileText, Video, ImageIcon, File } from "lucide-react"
          ...
          for (const level of foundCourse.levels) {
            for (const module of level.modules) {
              for (const submodule of module["lessons-submodules"] || []) {
                const lesson = (submodule.lessons || []).find((l) => l.id === params.lessonId)
                if (lesson) {
                  foundLesson = lesson
                  break
                }
      ...
      if (user) {
        try {
          ...
          if (response.ok) {
            ...
            setWatchedLessons(data.watchedLessons.map((item: any) => item.lessonId))
          }
        } catch (error) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <Button onClick={() => router.push(`/course/${params.courseId}`)}>Voltar para o curso</Button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container py-8 px-4 mx-auto max-w-6xl">
        <Button variant="ghost" className="mb-4" onClick={() => router.push(`/course/${params.courseId}`)}>
              ...
              <div className="mt-4">
                ...
                <div className="flex items-center mt-2">
                  <Button
                    ...
                    onClick={() => handleLessonComplete(currentLesson.id, !isLessonWatched(currentLesson.id))}
                  >
                  ...
                  {course.levels.flatMap((level) =>
                    level.modules.map((module) => (
                      <AccordionItem key={module.id} value={module.id}>
                        ...
                        <AccordionContent>
                          {(module["lessons-submodules"] || []).map((submodule) => (
                            <div key={submodule.id} className="pl-4 pr-2 py-2">
                              <h4 className="text-sm font-medium mb-2">{submodule.title}</h4>
                                ...
                                {(submodule.lessons || []).map((lesson) => (
                                  <li
                                    ...
                                    className={`flex items-center justify-between p-2 rounded-md ${
                                      lesson.id === params.lessonId ? "bg-muted" : ""
                                    }`}
                                  >
                                    ...
                                    <Button
                                      ...
                                      onClick={() => router.push(`/course/${params.courseId}/lesson/${lesson.id}`)}
                                    >
                                      {lesson.id === params.lessonId ? "Atual" : "Ver"}
                                    </Button>
                                  </li>
                                ))}
                              </ul>
